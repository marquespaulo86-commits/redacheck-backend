...
// ── PROMPT DE TRANSCRIÇÃO PURA (rotina professor, passada 1) ──────────
// Só transcreve. Não avalia, não corrige, não inventa. Base da arquitetura
// de duas passadas que elimina a confabulação em manuscritos.
const PROMPT_TRANSCRICAO = `A imagem contém uma redação manuscrita. Sua ÚNICA tarefa é TRANSCREVER fielmente o texto, exatamente como está escrito.\n\nREGRAS OBRIGATÓRIAS:\n- Transcreva palavra por palavra, letra por letra, exatamente como o aluno grafou. PRESERVE os erros do aluno: não corrija ortografia, acentuação, concordância nem nomes próprios.\n- NÃO avalie, NÃO comente, NÃO corrija e NÃO acrescente nada que não esteja escrito.\n- NUNCA invente palavras, nomes, autores, países, datas ou trechos. Se não conseguir ler um trecho com segurança, escreva exatamente "[ilegível]" no lugar — jamais chute uma palavra provável.\n- Preserve a divisão em parágrafos do original.\n\nResponda APENAS com o texto transcrito, sem título, sem aspas e sem nenhum comentário.`;
...
// ── TRANSCREVER (passada 1) — só transcreve a foto; cobra 1 crédito ───
// Fluxo professor de duas passadas. NÃO avalia aqui. Não toca no avulso.
app.post('/professor/transcrever', async (req, res) => {
  ...
  try {
    ...
    const MIME_VALIDOS = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
    const mimeRaw = mediaType || 'image/jpeg';
    if (mimeRaw === 'application/pdf') return res.status(400).json({ erro: 'PDF não suportado no modo foto. Use JPG/PNG.' });
    const mimeFinal = MIME_VALIDOS.includes(mimeRaw) ? mimeRaw : 'image/jpeg';
    if (imagem.length > 6_000_000) return res.status(400).json({ erro: 'Imagem muito grande. Reduza a resolução.' });

    // Debita 1 crédito do professor (cobre transcrição + avaliação da mesma redação)
    const debito = await pool.query(
    ...
    // Passada 1: SÓ transcreve (com 1 retry em sobrecarga)
    let transcricao = '';
    try {
      let data = null;
      for (let tentativa = 1; tentativa <= 2; tentativa++) {
        const response = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({
            model: 'claude-sonnet-4-5',
            max_tokens: 4000,
            system: 'Você transcreve manuscritos com fidelidade absoluta. Nunca inventa, corrige ou avalia nada.',
        ...
        if (!response.ok) {
          const corpo = await response.text().catch(() => '');
          console.error(`[/professor/transcrever] API HTTP ${response.status}: ${corpo.slice(0,200)}`);
          if ((response.status === 429 || response.status >= 500) && tentativa === 1) { await new Promise(r => setTimeout(r, 1500)); continue; }
          throw new Error(`API indisponível (HTTP ${response.status})`);
        }
        data = await response.json();
        if (data.error) {
          const tipo = data.error.type || '';
          if ((tipo === 'overloaded_error' || tipo === 'rate_limit_error' || tipo === 'api_error') && tentativa === 1) { await new Promise(r => setTimeout(r, 1500)); continue; }
          throw new Error(data.error.message || tipo || 'erro da API');
        }
        break;
      }
      if (!data || !data.content || !data.content[0]) throw new Error('resposta vazia da API');
      transcricao = (data.content[0].text || '').trim();
      if (transcricao.length < 20) throw new Error('transcrição vazia ou ilegível');
    } catch (apiErr) {
      await estornarCredito(professorId);
      console.error('[/professor/transcrever] FALHA:', apiErr.message);
      return res.status(502).json({ erro: 'Falha na transcrição (crédito devolvido): ' + apiErr.message });
    }

    // Salva a transcrição pendente (liga a passada 2 e evita cobrar de novo)
    ...
    try {
      const ins = await pool.query(
        ...
        [professorId, aluno.id, aluno.nome, aluno.instituicao || null, bancaFinal, transcricao]
      );
      transcricaoId = ins.rows[0]?.id;
    } catch (dbErr) {
      console.error('[/professor/transcrever] erro DB (segue sem persistir):', dbErr.message);
    }

    await log(professorId, null, 'professor_transcrever_ok', 'ok', { aluno_id: aluno.id, banca: bancaFinal, transcricao_id: transcricaoId });
    return res.json({
      transcricaoId, transcricao, banca: bancaFinal,
      aluno: { id: aluno.id, nome: aluno.nome, instituicao: aluno.instituicao || null },
      avaliacoes_disponiveis: saldoAposDebito
    });
  } catch (err) {
...
// ── AVALIAR TEXTO (passada 2) — avalia o texto confirmado; NÃO cobra ──
// Roda em modo TEXTO (sem imagem) → sem confabulação. Crédito já pago na passada 1.
app.post('/professor/avaliar-texto', async (req, res) => {
  try {
    ...
    // Recupera aluno/banca da transcrição (não cobra de novo)
    ...
    // Avaliação sobre TEXTO (sem imagem, sem TRANSCRICAO_FIEL)
    ...
    try {
      ...
      for (let tentativa = 1; tentativa <= 2; tentativa++) {
        ...
        if (!response.ok) {
          const corpo = await response.text().catch(() => '');
          console.error(`[/professor/avaliar-texto] API HTTP ${response.status}: ${corpo.slice(0,200)}`);
          if ((response.status === 429 || response.status >= 500) && tentativa === 1) { await new Promise(r => setTimeout(r, 1500)); continue; }
          throw new Error(`API indisponível (HTTP ${response.status})`);
        }
        data = await response.json();
      ...
      try {
        ...
        if (a >= 0 && b > a) limpo = limpo.substring(a, b + 1);
        avaliacaoJSON = JSON.parse(limpo);
      } catch {
        avaliacaoJSON = repararJSONTruncado(textoResposta);
        if (!avaliacaoJSON) throw new Error('resposta ilegível (JSON inválido/truncado)');
      ...
      // Crédito já foi pago na transcrição; não estorna aqui — professor pode reeditar e reenviar
    ...
    try {
      const ins = await pool.query(
        ...
        [professorId, aluno.id, aluno.nome, aluno.instituicao || null, bancaFinal,
         avaliacaoJSON.notaGeral || 0, JSON.stringify(avaliacaoJSON), textoFinal.substring(0, 5000)]
      );
      ...
      await log(professorId, null, 'professor_avaliar_texto_ok', 'ok', { aluno_id: aluno.id, avaliacao_id: ins.rows[0]?.id, banca: bancaFinal, nota_geral: avaliacaoJSON.notaGeral || 0 });
      return res.json({
        avaliacao: avaliacaoJSON, formato: 'json', banca: bancaFinal,
        aluno: { id: aluno.id, nome: aluno.nome, instituicao: aluno.instituicao || null }
      });
      ...
      return res.json({
        avaliacao: avaliacaoJSON, formato: 'json', banca: bancaFinal,
        aluno: { id: aluno.id, nome: aluno.nome, instituicao: aluno.instituicao || null },
        aviso: 'Avaliação realizada, mas não salva no histórico.'
      });
    }
  } catch (err) {
