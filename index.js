...
app.post('/avaliar', async (req, res) => {
  try {
    ...
    if (temImagem) {
      ...
      const NOTA_OCR = '\n\nOBSERVAÇÃO DE ORIGEM (importante): o texto acima veio de leitura automática (OCR) de um manuscrito, não foi digitado pelo aluno. Portanto, seja CONSERVADOR na Competência 1: NÃO trate como erro de ortografia/acentuação do aluno palavras isoladas que possam ser ruído de leitura do OCR, e NUNCA cite como desvio um trecho marcado "[ilegível]". Avalie normalmente estrutura, argumentação, coesão e proposta de intervenção.';
      mensagemConteudo = `${promptSistema}\n${SCHEMA_JSON}\n\nAvalie para a banca ${bancaFinal}:\n\nREDAÇÃO:\n${ocr.texto}${NOTA_OCR}`;
    }
